/* generated from /home/hareesh/seL4test/kernel/include/arch/x86/arch/object/structures.bf */
...
static inline uint64_t PURE
pte_ptr_get_xd(pte_t *pte_ptr) {
    uint64_t ret;
    ret = (pte_ptr->words[0] & 0x8000000000000000ull) >> 63;
    /* Possibly sign extend */
    if (__builtin_expect(!!(0 && (ret & (1ull << (50)))), 0)) {
        ret |= 0x0;
    }
...
static inline uint64_t PURE
pte_ptr_get_cache_disabled(pte_t *pte_ptr) {
    uint64_t ret;
    ret = (pte_ptr->words[0] & 0x10ull) >> 4;
    /* Possibly sign extend */
    if (__builtin_expect(!!(0 && (ret & (1ull << (50)))), 0)) {
        ret |= 0x0;
    }
...
static inline uint64_t PURE
pte_ptr_get_super_user(pte_t *pte_ptr) {
    uint64_t ret;
...
static inline uint64_t PURE
pte_ptr_get_read_write(pte_t *pte_ptr) {
    uint64_t ret;
...
static inline uint64_t PURE
pde_pde_large_ptr_get_xd(pde_t *pde_ptr) {
    uint64_t ret;
    assert(((pde_ptr->words[0] >> 7) & 0x1) ==
           pde_pde_large);

    ret = (pde_ptr->words[0] & 0x8000000000000000ull) >> 63;
    /* Possibly sign extend */
    if (__builtin_expect(!!(0 && (ret & (1ull << (50)))), 0)) {
        ret |= 0x0;
    }
...
static inline uint64_t PURE
pde_pde_large_ptr_get_cache_disabled(pde_t *pde_ptr) {
    uint64_t ret;
    assert(((pde_ptr->words[0] >> 7) & 0x1) ==
           pde_pde_large);

    ret = (pde_ptr->words[0] & 0x10ull) >> 4;
    /* Possibly sign extend */
    if (__builtin_expect(!!(0 && (ret & (1ull << (50)))), 0)) {
        ret |= 0x0;
    }
...
static inline uint64_t PURE
pde_pde_large_ptr_get_super_user(pde_t *pde_ptr) {
    uint64_t ret;
    assert(((pde_ptr->words[0] >> 7) & 0x1) ==
           pde_pde_large);

    ret = (pde_ptr->words[0] & 0x4ull) >> 2;
    /* Possibly sign extend */
    if (__builtin_expect(!!(0 && (ret & (1ull << (50)))), 0)) {
        ret |= 0x0;
    }
...
static inline uint64_t PURE
pde_pde_large_ptr_get_read_write(pde_t *pde_ptr) {
    uint64_t ret;
    assert(((pde_ptr->words[0] >> 7) & 0x1) ==
           pde_pde_large);

    ret = (pde_ptr->words[0] & 0x2ull) >> 1;
    /* Possibly sign extend */
    if (__builtin_expect(!!(0 && (ret & (1ull << (50)))), 0)) {
        ret |= 0x0;
    }
...
static inline uint64_t PURE
pdpte_pdpte_1g_ptr_get_xd(pdpte_t *pdpte_ptr) {
    uint64_t ret;
    assert(((pdpte_ptr->words[0] >> 7) & 0x1) ==
           pdpte_pdpte_1g);

    ret = (pdpte_ptr->words[0] & 0x8000000000000000ull) >> 63;
    /* Possibly sign extend */
    if (__builtin_expect(!!(0 && (ret & (1ull << (50)))), 0)) {
        ret |= 0x0;
    }
...
static inline uint64_t PURE
pdpte_pdpte_1g_ptr_get_cache_disabled(pdpte_t *pdpte_ptr) {
    uint64_t ret;
    assert(((pdpte_ptr->words[0] >> 7) & 0x1) ==
           pdpte_pdpte_1g);

    ret = (pdpte_ptr->words[0] & 0x10ull) >> 4;
    /* Possibly sign extend */
    if (__builtin_expect(!!(0 && (ret & (1ull << (50)))), 0)) {
        ret |= 0x0;
    }
...
static inline uint64_t PURE
pdpte_pdpte_1g_ptr_get_super_user(pdpte_t *pdpte_ptr) {
    uint64_t ret;
    assert(((pdpte_ptr->words[0] >> 7) & 0x1) ==
           pdpte_pdpte_1g);

    ret = (pdpte_ptr->words[0] & 0x4ull) >> 2;
    /* Possibly sign extend */
    if (__builtin_expect(!!(0 && (ret & (1ull << (50)))), 0)) {
        ret |= 0x0;
    }
...
static inline uint64_t PURE
pdpte_pdpte_1g_ptr_get_read_write(pdpte_t *pdpte_ptr) {
    uint64_t ret;
    assert(((pdpte_ptr->words[0] >> 7) & 0x1) ==
           pdpte_pdpte_1g);

    ret = (pdpte_ptr->words[0] & 0x2ull) >> 1;
    /* Possibly sign extend */
    if (__builtin_expect(!!(0 && (ret & (1ull << (50)))), 0)) {
        ret |= 0x0;
    }
